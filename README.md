...
### Phase 8~9: RAG 파이프라인 추가 및 최적화 (50명, 5분)

| 버전 | /input p50 | /input p95 | RPS | 에러율 |
|---|---|---|---|---|
| no_cache (기준) | 1,500ms | 2,800ms | 16.9 | 2.2% |
| RAG + LLM 의도분류 | 2,800ms | 5,100ms | 14.5 | **22.9%** |
| **RAG + 키워드 의도분류** | **2,100ms** | **3,600ms** | **17.6** | **1.9%** |

### 개선 과정 요약

**캐싱 전략:**
- Step 1 (완전 일치): 히트율 11.5%, 효과 제한적
- Step 2 (임베딩 naive): Redis O(N) 스캔으로 p50 4,600ms 악화
- Step 3 (pgvector): HNSW 인덱스로 히트율 18.2%, 응답시간 안정화

**RAG 최적화:**
- 문제: LLM 의도 분류가 요청마다 LLM 1회 추가 호출 → 에러율 22.9% 급증
- 원인: OpenAI RPD 10,000건/일 한도 초과 (RAG는 LLM 2회 호출)
- 해결: 키워드 기반 의도 분류로 교체 (200ms → 1ms, LLM 호출 절감)
- 결과: 에러율 22.9% → 1.9%, RPS no_cache 수준 회복
